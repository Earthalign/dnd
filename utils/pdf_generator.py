...
def generate_character_pdf(char_data: dict, output_path: str):
    """
    Wypełnia kartę postaci D&D 5e (karta-postaci-interaktywna.pdf) używając pymupdf.
    Mapowanie pól zostało w pełni zweryfikowane wizualnie na zrenderowanym szablonie.
    """
    base_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    template_path = os.path.join(base_dir, "karta-postaci-interaktywna.pdf")

    if not os.path.exists(template_path):
        raise FileNotFoundError(
            f"Brakuje pliku szablonu: {template_path}. "
            "Umieść plik 'karta-postaci-interaktywna.pdf' w folderze projektu dnd/."
        )
